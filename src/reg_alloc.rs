use crate::Reg;
use crate::{AsmOp, OpTarget, CallType};
use crate::Loc;
...
impl RegAlloc {
    ...
    fn evict_reg(&mut self, reg: u8) {
        ...
        if let Some(var) = self.regs[idx] {
            ...
            if let Some(free_reg) = self.free_reg() {
                ...
                self.vars.insert(var, VarLoc::Reg { reg: free_reg as u8, stack_slot: *stack_slot });
                self.opqueue.push(AsmOp::Mov(OpTarget::Reg(free_reg as u8), OpTarget::Reg(reg)));
                self.opqueue.push(AsmOp::Comment(format!(" ^^^ L_{var} from {reg:?} to free", reg = Reg::from_idx(reg))));
            } else {
                ...
                self.opqueue.push(AsmOp::Comment(format!(" ^^^ L_{var} to stack")));
            }
        }
    ...
    fn oldest(&self) -> u8 {
        self.times.iter().enumerate().min_by_key(|&(_i, &a)| a).expect("nonempty array").0 as u8
    }
    ...
    fn oldest_protected(&self, protected: &[u8]) -> u8 {
        self.times.iter()
            ...
            .min_by_key(|&(_i, &a)| a)
            .expect("nonempty array and protected leaves room").0 as u8
    }
            ...
            match curr {
                VarLoc::Stack(slot) => {
                    ...
                    *curr = VarLoc::Reg { reg: target, stack_slot: Some(slot) };
                    ...
                    self.opqueue.push(AsmOp::Comment(format!(" ^^^ L_{var} := *(rsp + {slot})")));
                },
                VarLoc::Reg { reg, .. } => {
                    // self.opqueue.push(AsmOp::Comment(format!(" vvv L_{var} moving to {reg:?}", reg = Reg::from_idx(target))));
                    if *reg == target {
                        // already in the right spot, don't need to do anything
                        return;
                    }
                    // self.opqueue.push(AsmOp::Comment(
                    //     format!("current = {reg:?} vs target = {target:?}",
                    //         reg = Reg::from_idx(*reg),
                    //         target = Reg::from_idx(target))));
                    let prev = *reg;
                    self.evict_reg(target);
                    ...
                    *reg = target;
                    ...
                    self.opqueue.push(AsmOp::Mov(OpTarget::Reg(target), OpTarget::Reg(src)));
                    self.opqueue.push(AsmOp::Comment(format!(" ^^^ L_{var} := {reg:?}", reg = Reg::from_idx(src))));
                },
                VarLoc::Uninit => todo!(),
            };
        } else {
    ...
    pub fn free(&mut self, var: Loc) {
        ...
        self.opqueue.push(AsmOp::Comment(format!("( free L_{var} )")));
